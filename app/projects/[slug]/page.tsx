...
async function getProject(slug: string) {
  ...
  // 여기서 slug가 제대로 안 들어오면 에러가 납니다.
  return client.fetch(query, { slug });
}

// 3. 페이지 컴포넌트
// ⚠️ 중요: Next.js 15+ 에서는 params가 Promise 타입입니다.
export default async function ProjectPage({ 
  params 
}: { 
  params: Promise<{ slug: string }> 
}) {
  // 🌟 핵심 수정: params를 먼저 await로 기다려야 값을 꺼낼 수 있습니다.
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex gap-2 mb-6 flex-wrap">
            {project.tags?.map((tag) => (
              <span key={tag} className="px-3 py-1 text-sm font-medium rounded-full bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300">
                {tag}
              </span>
            ))}
          </div>
